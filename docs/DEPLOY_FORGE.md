...
3. Set the **Deploy script** to `bash scripts/forge-deploy.sh` (included in
   this repo).
...
| Deploy fails on `go build` | Install Go on server: Forge → Server → **Meta** → ensure Go is available, or install via `Server → Recipes` |
